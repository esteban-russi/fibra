...
import { JOURNAL_ID, SLOW_DESCENT_MS, scrollToElement } from '../lib/scroll'
...
export function Home() {
  ...
  // Arriving from another route with the cover's section hash — the journal.
  // RouteChange has just reset the scroll to the top, so the section is taken
  // directly: the eased descent is for visitors already on this page, where the
  // travel is the point. Across a route change it would only be 2.4s of content
  // nobody asked to see.
  const { hash } = useLocation()
  useEffect(() => {
    const id = hash.slice(1)
    if (id !== JOURNAL_ID) return
